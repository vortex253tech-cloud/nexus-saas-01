...
7. ~~Unificar (ou confirmar que já estão unificados) o pipeline de vendas do WhatsApp V5 com o módulo de Leads/CRM geral~~ — **Parcialmente feito em 2026-06-18.** A investigação revelou um problema mais profundo: o CRM inteiro (13 arquivos) usava uma única empresa fixa (`NEXUS_PLATFORM_COMPANY_ID`) em vez da empresa do usuário logado — corrigido (decisão explícita do usuário: preparar para multi-tenant real desde já). Detalhe completo em [decisoes.md](./decisoes.md). **Ainda falta:** o pedido original — sincronizar `lead_context`/`whatsapp_conversations` com a tabela `leads` (existe um `syncToLeads()` de referência em `app/api/whatsapp/webhook/route.ts` que faz esse mapeamento, só nunca foi chamado pelo pipeline real).
