...
| `lead_context`/`whatsapp_conversations` não sincronizam com a tabela `leads` | 🟡 Médio, pendente | Pedido original do item 7 — ainda não implementado. Existe lógica de referência (`syncToLeads()`) em `app/api/whatsapp/webhook/route.ts`, só nunca foi conectada ao pipeline real (`analyze/route.ts`). |
