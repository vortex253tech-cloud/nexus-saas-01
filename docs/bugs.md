...
| `financial-insights` e `creative/generate` confiam em `company_id` do corpo da requisição | 🟠 Alto, novo achado (não é plan-gating) | Usuário autenticado de uma empresa pode, em teoria, passar `company_id` de outra empresa nessas rotas — gap de isolamento multi-tenant (IDOR), não corrigido nesta rodada (fora do escopo de gating). |
