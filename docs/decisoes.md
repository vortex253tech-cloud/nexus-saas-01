...
**Atualização ainda no mesmo dia — template redesenhado pra bater com o padrão visual de referência.** Usuário mandou print de um post manual antigo (foto real de profissional, headline grande alinhada à esquerda, subtítulo entre parênteses, logo NEXUS + @nexus.saas.ia no canto inferior esquerdo, "deslize →" + contador de slide no canto inferior direito) e pediu pra próximas imagens seguirem esse padrão. `lib/image-text-overlay.ts` reescrito (layout, vinheta esquerda-direita, lockup de marca, contador de carrossel), canvas mudou de 1024x1024 pra 1080x1350 (4:5, padrão do feed/carrossel do Instagram), e todos os 9 ângulos tiveram o prompt de imagem trocado de ícones/auras abstratas pra fotografia editorial realista de um profissional de negócios. Testado localmente (gpt-image-1 real + overlay) antes de subir — resultado visualmente muito próximo da referência. Primeiro post no template novo: https://www.instagram.com/p/DZ-4NnwjtCR/.

**Bug real descoberto pelo usuário no post seguinte:** a imagem publicada não tinha NENHUM texto — nem headline, nem subtítulo, nem "NEXUS"/"@nexus.saas.ia" — só o quadrado roxo vazio da logo. Causa: texto SVG (`<text>`) renderizado via `sharp` depende do sistema operacional ter fontconfig + fontes instaladas; o runtime serverless da Vercel não tem nenhuma. Formas (`<rect>`, gradientes) continuavam aparecendo normalmente, só o texto desaparecia silenciosamente — sem erro, sem warning. Corrigido substituindo SVG por `@napi-rs/canvas` (renderizador próprio, Skia, não depende de fonte do sistema) + 3 arquivos de fonte Inter (ExtraBold/Medium/Regular, ~68KB cada) empacotados em `assets/fonts/`, registrados via `GlobalFonts.registerFromPath`, com `outputFileTracingIncludes` no `next.config.js` garantindo que a Vercel realmente inclua esses arquivos no bundle da function (o tracer não detecta `fs.readFileSync` com caminho dinâmico automaticamente). Confirmado publicando um post real e baixando a imagem de volta do Instagram: https://www.instagram.com/p/DZ-7nscGgu0/ — texto e logo aparecem corretamente.
...
**Preço do Enterprise estava divergente do Stripe.** Ao auditar os 6 Price IDs (`STRIPE_*_MONTHLY/ANNUAL`) via API do Stripe antes do lançamento, todos vieram `active: true`, mas o valor do plano Scale/Enterprise não batia: Stripe cobra R$797/mês (R$597,75 equivalente anual), a página `/planos` mostrava R$897/R$697. Starter e Pro batiam exatamente. Padrão de desconto anual dos outros 2 planos é ~25% (197→147, 397→297) — 797→597 segue o mesmo padrão, 897→697 não (~22%), indicando que o texto da página tinha divergido do preço real configurado, não o contrário. Confirmado com o usuário — `/planos` corrigido para 797/597. Sem essa checagem, qualquer cliente que assinasse o Enterprise via checkout seria cobrado R$100/mês menos do que o anunciado — discrepância anúncio vs. cobrança que ninguém tinha notado.

**CNPJ pendente, deliberadamente.** Termos de Uso/Privacidade (ver entrada anterior) ainda têm `[RAZÃO SOCIAL]`/`[CNPJ]` como placeholder — usuário confirmou que ainda não tem CNPJ aberto e optou por deixar como pendência conhecida por enquanto, não bloqueia o restante do lançamento.
